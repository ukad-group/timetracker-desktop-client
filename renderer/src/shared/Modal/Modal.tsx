...
const Modal = ({ isOpen, children, onSubmit, title, onClose, initialFocus }: ModalProps) => (
  <Transition appear={true} show={isOpen} as={Fragment}>
    <Dialog
      ...
      onFocus={(event) => {
        if (initialFocus?.current && event.target === event.currentTarget) {
          ...
          >
            ...
            <div className="mt-6 flex justify-end">
              <div className="flex flex-row-reverse gap-3">
                <Button text="Save" type={"submit"} status={"enabled"} />
                <Button text="Cancel" type={"button"} callback={onClose} status={"cancel"} />
              </div>
            </div>
